...
Run MATEK inside the existing Git project that should own the research artifacts. For formal
...
matek run problem.md --no-lean

# Research only; no manuscript or Lean formalization
matek run problem.md --research-only

# Disable model web search and MATEK's public-identifier HTTP lookups
matek run problem.md --no-web-search --research-only

# Cap total active workflow time, including later resume attempts
matek run problem.md --time-limit-minutes 180
...
reasoning setting. MATEK implements that behavior explicitly: one durable GPT 5.6 Sol logical
...
reasoning-effort controls, which do not expose that Responses API field as a separate MATEK
setting. The coordinator is restored from MATEK's canonical on-disk scheduler checkpoint, so it
...
max_wall_clock_minutes = 900  # 15 active hours across run/resume
...
does not enable Codex nested tools; `--flat` is the direct CLI override. The Responses API adapter
visibly resolves to portable flat execution because it has no nested-agent tool. See the official
...
keys are deprecated compatibility inputs. MATEK translates them into coordinator-decision and
...
MATEK has two orchestration layers. The outer `WorkflowRunner` is deterministic application
...
Coordinator transport defaults to a conservative 800,000-character hard ceiling measured after
the backend has serialized its final input. Compact/indexed requests target at most 760,000
characters, reserving at least 5% or 40,000 characters of headroom. Compaction never byte-truncates
JSON or mathematics. Each
activation persists a manifest with included and omitted artifact IDs, validated relative paths,
hashes, graph revisions, per-section character measurements, character/token estimates, and the
exact payload hash. The compact request includes only high-priority catalog entries plus the full
catalog's path, SHA-256, total count, and kind counts. Its graph section is one root/revision/index
descriptor plus the already bounded node summaries—never a duplicate exhaustive graph view.
Codex can inspect catalogued files when deeper evidence is needed; an API coordinator can request
up to 32 omitted artifacts or graph nodes for its next bounded activation. A provider size
rejection creates a smaller, distinct request. If cumulative scheduler history still does not fit, MATEK
...
knowledge graph. `MANDATORY_CONTEXT_TOO_LARGE` is reserved for the exact prompt/claim plus provider
...
If a worker reports a complete proof, MATEK pauses admission of new workers and packages the
...
The persistent problem memory is deliberately outside every run but still inside MATEK's safe
write boundary:
...
`matek_*` and other typed frontmatter are machine-owned. Conflicting edits fail validation rather
...
When an existing theorem already solves the problem, MATEK may continue by independently
...
MATEK is primarily validated on Linux. On WSL2, install and run both Codex and MATEK inside
...
By default, MATEK invokes the locally installed official Codex CLI and reuses the login created
by `codex login`. With **Sign in with ChatGPT**, MATEK neither needs nor stores an OpenAI
Platform API key.

This is not offline or local-model execution: Codex communicates with OpenAI and consumes the
Codex allowance or credits available to the signed-in ChatGPT account or workspace. MATEK does
...
| Command | Purpose |
| --- | --- |
| `matek init [--force]` | Create `matek.toml`, `.matek/`, and `problem.example.md` |
...
Ordinary `matek doctor` sends no model prompt. `--deep` explicitly opts into one minimal live
...
Paused time between commands is not charged. The default is 900 minutes (15 active hours);
`MATEK_TIME_LIMIT_MINUTES=N` is equivalent to the CLI option. Existing configurations that set
...
`matek init` creates a configuration with these Codex defaults:
...
Backend selection precedence is an explicit `--backend` flag, `MATEK_BACKEND`, project
...
migrated to the namespaced `[api]` layout and retain API behavior. MATEK prints a one-time
migration notice and does not discard settings or run state.

For safety, `codex.extra_args` accepts only the documented presentation allowlist; MATEK owns
...
matek doctor --online
matek run problem.md --backend api
```
...
page](https://developers.openai.com/api/docs/pricing). Never put the key in `matek.toml`.

### Safety, provenance, and reproducibility

- MATEK writes only inside `.matek/` unless `--allow-project-edits` is supplied and recorded.
...
- Failed verification gates remain failed. MATEK does not silently weaken bibliography,
...
- **`codex` not found:** install or update it with the official Codex CLI guide, ensure it is on
  `PATH`, then run `matek doctor`.
- **Not signed in:** run `codex login`, choose **Sign in with ChatGPT**, confirm with
  `codex login status`, then rerun `matek doctor`.
- **Unsupported Codex CLI:** MATEK checks the exact noninteractive, JSONL, schema, sandbox,
...
- **Model unavailable or reasoning effort rejected:** choose an explicit model and effort
  available to the workspace, then resume the checkpointed run. MATEK does not inherit a
...
- **WSL2:** install Codex and MATEK inside WSL2 and keep the project in its Linux filesystem.
- **Use API mode intentionally:** configure `OPENAI_API_KEY` and pass `--backend api`; there is
  no automatic fallback from Codex.

## Support

If `matek doctor` and the troubleshooting guide do not resolve an issue, open a report in the
[GitHub issue tracker](https://github.com/PhillipKerger/matek-theorem-agent/issues). Include the command you
ran, the relevant diagnostic message, and the run's authoritative status. Remove private problem
content and never post credentials. MATEK is maintained through the canonical GitHub repository.
...
```bash
MATEK_CODEX_LIVE_TESTS=1 pytest -q -m codex_live
...
MATEK is available under the MIT License. See [`LICENSE`](LICENSE).
