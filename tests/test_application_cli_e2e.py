...
@pytest.mark.asyncio
async def test_force_stage_uses_new_model_cache_generation_and_preserves_old_records(
    tmp_path: Path,
) -> None:
    project = tmp_path / "project"
    project.mkdir()
    ...
    assert len(model.requests) == calls_before * 2
    assert forced.state.metadata["model_cache_generation"] == 1
    assert records_before < records_after
    assert len(records_after) == len(records_before) * 2
    assert len(forced.state.paid_call_ids) == calls_before * 2
    assert backend.calls == 0
    assert codex.calls == 0
